<div align="center"><center>

<img alt="Icon" width=100 src="https://raw.githubusercontent.com/TerminalMC/ClaimPoints/HEAD/common/src/main/resources/assets/claimpoints/icon.png">

## ClaimPoints

Integrates with [Xaero's Minimap](https://modrinth.com/mod/1bokaNcj) and the
[GriefPrevention](https://modrinth.com/mod/O4o4mKaq) server plugin to allow semi-automatic tracking of 
claims using waypoints.

[![Environment](https://img.shields.io/badge/Environment-Client-blue?logo=data:image/png;base64,)]()
[![Latest Minecraft](https://img.shields.io/modrinth/game-versions/dyuTci89?label=Latest%20Minecraft&color=%2300AF5C&logo=data:image/png;base64,)](https://modrinth.com/mod/dyuTci89/versions)

[![Loader](https://img.shields.io/badge/Available%20for-Fabric-dbd0b4?logo=data:image/png;base64,)](https://fabricmc.net/)
[![Loader](https://img.shields.io/badge/Available%20for-NeoForge-f16436?logo=data:image/png;base64,)](https://neoforged.net/)

[![Download on Modrinth](https://img.shields.io/modrinth/dt/dyuTci89?label=Download%20on%20Modrinth&logo=modrinth&logoColor=%2300AF5C)](https://modrinth.com/mod/dyuTci89)
[![Download on CurseForge](https://img.shields.io/curseforge/dt/1049887?label=Download%20on%20CurseForge&logo=curseforge)](https://curseforge.com/minecraft/mc-mods/claimpoints)
[![Download on GitHub](https://img.shields.io/github/downloads/TerminalMC/ClaimPoints/total?label=Download%20on%20GitHub&logo=github&logoColor=white)](https://github.com/TerminalMC/ClaimPoints)

</center></div>

### About

- This is a client-side mod, so it does not allow you to create or edit claims, only allows easy visualization of where 
your existing claims are.

- Use `/cp help` in-game for command info.

- If, when running the `/cp` commands, the GriefPrevention claim list shows up in chat, you will need to edit the regex
patterns in the config to allow the mod to correctly capture the claim list.

<table style="width:100%;">
  <tr>
    <td style="width:50%;"><img src="https://raw.githubusercontent.com/TerminalMC/ClaimPoints/HEAD/assets/images/chat_01.png" style="width:100%;"></td>
    <td style="width:50%;"><img src="https://raw.githubusercontent.com/TerminalMC/ClaimPoints/HEAD/assets/images/config_01.png" style="width:100%;"></td>
  </tr>
</table>

#### Dependencies

Fabric: [Xaero's Minimap](https://modrinth.com/mod/1bokaNcj), [Fabric API](https://modrinth.com/mod/P7dR8mSH), [ModMenu](https://modrinth.com/mod/mOgUt4GM), [Cloth Config API](https://modrinth.com/mod/9s6osm5g)

NeoForge: [Xaero's Minimap](https://modrinth.com/mod/1bokaNcj), [Cloth Config API](https://modrinth.com/mod/9s6osm5g)

### Contact

[![Discord Server](https://img.shields.io/discord/1103153365216669797?logo=discord&label=Discord%20Server&color=%235865F2)](https://discord.terminalmc.dev)

[![GitHub Issues](https://img.shields.io/github/issues/TerminalMC/ClaimPoints?logo=github&label=GitHub%20Issues)](https://github.com/TerminalMC/ClaimPoints/issues)

[![License](https://img.shields.io/github/license/TerminalMC/ClaimPoints?label=License&logo=github&logoColor=white)](https://github.com/TerminalMC/ClaimPoints/blob/HEAD/LICENSE.txt)